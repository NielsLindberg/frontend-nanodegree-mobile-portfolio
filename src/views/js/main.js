...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var numberOfPizzas = Math.floor(((viewportHeight + 156) / S)) * 8;

    for (var i = 0; i < numberOfPizzas; i++) {
        pizzArray.push(new Pizza(i, 'mover', 'images/pizza.png', '100px', '73.333px', (i % COLS) * S, (Math.floor(i / COLS) * S) + 'px'));
    }
    pizzArray.forEach(function(pizza, index) {
        pizza.addToDom();
    });
    updatePositions();
});
